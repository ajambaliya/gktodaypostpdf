...
IGNORE_URL_PATTERN = "https://www.gktoday.in/daily-current-affairs-quiz-"
...
def check_and_insert_urls(urls):
    new_urls = []
    for url in urls:
        if IGNORE_URL_PATTERN not in url and not collection.find_one({'url': url}):
            new_urls.append(url)
...
async def send_pdf_to_telegram(pdf_path, bot_token, channel_id):
    ...
    max_retries = 3
    file_name = f"{datetime.now().strftime('%d %B %Y')} Current Affairs.pdf"
    for attempt in range(max_retries):
        try:
            with open(pdf_path, 'rb') as pdf_file:
                await bot.send_document(chat_id=channel_id, document=pdf_file, filename=file_name,
                                        caption=f"🎗️ {datetime.now().strftime('%d %B %Y')} Current Affairs 🎗️\n\n"
                                                f"👉 English Titles of scraped Links\n\n"
                                                f"🎉 Join us :- @CurrentAdda 🎉")
            break
